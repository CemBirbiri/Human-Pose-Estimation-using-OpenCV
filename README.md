...
In this project, human pose estimation of people in a video clip was applied to draw bounding boxes of body and face and lines for legs and arms. A JSON file was created using the [OpenPose pose estimator](https://cmu-perceptual-computing-lab.github.io/openpose/web/html/doc/). OpenPose has represented the first real-time multi-person system to jointly detect human body, hand, facial, and foot key points (in total 135 key points) on single images. It returns a JSON file where the coordinates of the face and body parts are given. Using these coordinates one can draw bounding boxes of the face and body as well as lines for legs and arms.
...
2. Read the JSON file coming from the [OpenPose pose estimator](https://cmu-perceptual-computing-lab.github.io/openpose/web/html/doc/), and detect the coordinates of the face, body, arms, and legs.
